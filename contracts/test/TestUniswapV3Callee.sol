...
contract TestUniswapV3Callee is IUniswapV3MintCallback, IUniswapV3SwapCallback, IUniswapV3FlashCallback {
    ...
    function mint(
        ...
        IUniswapV3Pool(pool).mint{value: msg.value}(recipient, tickLower, tickUpper, amount, abi.encode(msg.sender));
    }
    ...
    function uniswapV3MintCallback(
        ...
        if (amount0Owed > 0) {
            ...
            TransferHelper.safeTransfer(IUniswapV3Pool(msg.sender).token0(), sender, msg.sender, amount0Owed);
        }

        if (amount1Owed > 0) {
            // IERC20(IUniswapV3Pool(msg.sender).token1()).transferFrom(sender, msg.sender, amount1Owed);
            TransferHelper.safeTransfer(IUniswapV3Pool(msg.sender).token1(), sender, msg.sender, amount1Owed);
        }
    ...
    function uniswapV3FlashCallback(
        ...
        if (pay0 > 0) TransferHelper.safeTransfer(IUniswapV3Pool(msg.sender).token0(), sender, msg.sender, pay0);
        if (pay1 > 0) TransferHelper.safeTransfer(IUniswapV3Pool(msg.sender).token1(), sender, msg.sender, pay1);
    }
}
