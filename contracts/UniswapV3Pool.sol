...
import {HbarConversion} from './libraries/HbarConversion.sol';
...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    constructor() {
        ...
        AssociateHelper.safeAssociateTokens(address(this), tokens);
    }
    ...
    function mint(
        ...
        uint128 amount,
        bytes calldata data
    ) external payable override lock returns (uint256 amount0, uint256 amount1) {

        uint256 requiredTinybars = HbarConversion.tinycentsToTinybars(IUniswapV3Factory(factory).mintFee());
    ...
    function collect(
        ...
        unchecked {
            if (amount0 > 0) {
                ...
                TransferHelper.safeTransfer(token0, address(this), recipient, amount0);
            }
            if (amount1 > 0) {
                position.tokensOwed1 -= amount1;
                TransferHelper.safeTransfer(token1, address(this), recipient, amount1);
            }
        }
    ...
    function swap(
        ...
        if (zeroForOne) {
            unchecked {
                if (amount1 < 0) TransferHelper.safeTransfer(token1, address(this), recipient, uint256(-amount1));
            }
            ...
            unchecked {
                if (amount0 < 0) TransferHelper.safeTransfer(token0, address(this), recipient, uint256(-amount0));
            }
    ...
    function flash(
        ...
        if (amount0 > 0) TransferHelper.safeTransfer(token0, address(this), recipient, amount0);
        if (amount1 > 0) TransferHelper.safeTransfer(token1, address(this), recipient, amount1);
    ...
    function collectProtocol(
        ...
        IUniswapV3Factory(factory).rentPayer().call{value: address(this).balance}("");
